...
export function isSupportedCountry(value: unknown): value is string {
  return typeof value === "string" && COUNTRY_SET.has(value.toUpperCase());
}
...
export function countryLabel(value: string, locale: "ko" | "en") {
  const code = value.toUpperCase();
  if (!isSupportedCountry(code)) return "";
  const name = new Intl.DisplayNames([locale === "ko" ? "ko" : "en"], { type: "region" }).of(code) || code;
