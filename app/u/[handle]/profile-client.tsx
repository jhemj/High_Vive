...
import { countryLabel } from "../../../packages/shared/countries";
...
type ProfilePassport = {
  ...
  evaluator: { model?: string; codexVersion?: string; agentVersion?: string; tools?: string[] };
  limitations: string[];
...
export function ProfileClient({ handle, locale }: { handle: string; locale: "ko" | "en" }) {
  ...
  const witnessName = passport.evaluator.tools?.includes("claude-code") ? "CLAUDE CODE WITNESS" : "CODEX WITNESS";
  ...
  return <main className="profile-page">
    ...
    <section className="profile-grid"><article className="profile-card narrative-card"><p className="eyebrow">{witnessName}</p><blockquote>“{passport.summary[locale]}”</blockquote><div className="assessment-split"><section><h2>{locale === "ko" ? "강점" : "Strengths"}</h2><ul>{passport.strengths[locale].map((item: string) => <li key={item}>{item}</li>)}</ul></section><section><h2>{locale === "ko" ? "보완점" : "Gaps"}</h2><ul>{passport.weaknesses[locale].map((item: string) => <li key={item}>{item}</li>)}</ul></section></div></article>
      ...
      <article className="profile-card evidence-card"><p className="eyebrow">EVIDENCE & LIMITS</p><dl><div><dt>LEVEL</dt><dd>{passport.evidenceLevel} · {passport.evidenceLabel}</dd></div><div><dt>PROTOCOL</dt><dd>{passport.protocolVersion}</dd></div><div><dt>MODEL</dt><dd>{passport.evaluator.model || witnessName.replace(" WITNESS", "")}</dd></div><div><dt>AGENT</dt><dd>{passport.evaluator.agentVersion || passport.evaluator.codexVersion || "reported locally"}</dd></div></dl><ul>{passport.limitations.map((item: string) => <li key={item}>{item}</li>)}</ul></article>
    </section>
    <footer className="site-footer"><p>{locale === "ko" ? "특정 기기와 시점에 발견된 로컬 AI 코딩 에이전트 이력 기반 AI Witness 평가입니다. 신원·전체 이력·실제 성과를 보증하지 않습니다." : "An AI Witness assessment of local AI coding-agent history found on one device at one time. It does not prove identity, complete history, or real-world outcomes."}</p></footer>
  </main>;
}
