...
High-Vive 서버로 전송되는 것은 사용자가 확인한 한국어·영어 공개 요약, 강점·보완점, 소수점 단위의 10개 원점수, 고정 분야, 사용 도구, 집계 범위와 evidence hash뿐이다. 서버는 원점수를 기준분포 백분위로 변환해 OVR을 계산하고, 평가 범위·기간·무결성으로 0–100 검증 신뢰도를 산출한다. 로컬 대화 원문, 파일 경로, 도구 인자, 명령 출력은 전송하지 않는다.
