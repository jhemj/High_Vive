...
High-Vive 서버로 전송되는 것은 사용자가 확인한 한국어·영어 공개 요약, 강점·보완점, 소수점 단위의 10개 원점수, 고정 분야, 사용 도구, 집계 범위와 evidence hash뿐이다. 서버는 버전이 고정된 보정 함수로 Calibrated OVR을 계산하고, 서버가 확인한 소유권·commitment·challenge·표본 proof로 0–100 Reliability를 산출한다. 로컬 대화 원문, 파일 경로, 도구 인자, 명령 출력은 전송하지 않는다.
