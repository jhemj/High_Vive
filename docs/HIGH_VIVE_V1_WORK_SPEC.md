...
- 서버 LLM 호출: 0회
- 공식 평가자: 사용자 로컬 환경의 Codex
- 문서 상태: 개발 착수 기준안

## 1. 제품 정의

High-Vive는 사용자의 로컬 Codex 작업 이력을 기반으로 Codex가 자기 사용자의 AI 협업 역량을 평가하고, 그 결과를 비교 가능한 Passport와 리더보드로 공개하는 바이브코더 벤치마크 플랫폼이다.
...
| High-Vive Web | Passport, 프로필, Official/Open 리더보드 |
...
포함 범위는 사용자·프로필·handle 소유권, 로컬 Codex 전체 이력 평가, commitment/challenge, Passport, 10개 지표, Calibrated OVR, HV Rating, Provisional Tier, Reliability, Evidence Level, Official/Open 리더보드, 프로필·평가 이력, audit, revoke, rate limit, 데이터 삭제, protocol versioning, 테스트·CI다.
...
- HV Rating: `round(Calibrated OVR × 10)`인 0–1000 벤치마크 rating
...
Official 조건은 E2 이상, Reliability 60 이상, current protocol, 소유 프로필 공개, non-demo, non-revoked다. 나머지는 Open 리더보드에만 표시한다.
...
v1.0에서는 실제 cohort percentile이라는 표현을 사용하지 않는다. `hv-calibration-v1` 규칙으로 계산한 값을 Calibrated Score와 Calibrated OVR로 표시한다. HV Rating은 OVR만으로 계산하며 Reliability를 섞지 않는다. 모든 Tier는 Provisional Tier다.
...
5. Codex는 정량 집계, 선택 표본, rubric, anchor, 공개 schema만 읽고 10개 지표와 근거를 평가한다.
6. 사용자가 공개·비공개 항목과 한계를 확인한다.
7. 서버가 소유권, 상태, nonce, 만료, version, root, payload hash, schema, replay, proof, PII를 검증한 뒤 append-only Passport를 저장한다.
...
메인은 Official/Open, 분야 필터, podium, 전체 순위, HV Rating, Provisional Tier, Calibrated OVR, Reliability, Evidence Level, 평가일, protocol version을 표시한다. `/u/{handle}`은 현재 Passport, 요약, 강점·보완점, Raw/Calibrated 10개 지표, 범위, evaluator metadata, 과거 평가, 성장 추이, 한계 문구를 표시한다.

공식 등록 UI에서 raw JSON textarea를 제거한다. CLI 명령, assessment 상태, commitment, 평가 완료, 공개 preview, 승인, publish 흐름만 제공한다. demo는 `is_demo = true`, DEMO 배지, Official·참가자 수 제외를 강제한다.
...
- EPIC 4: Codex Witness, rubric, evidence refs, metadata, preview
...
- EPIC 7: Official/Open, profile, history, growth, onboarding
- EPIC 8: rate limit, PII, audit, deletion, threat model
- EPIC 9: unit, integration, scanner, security, E2E, CI, migration

Gate A는 protocol·소유권·신규 schema·상태 모델·legacy migration, Gate B는 scan·commitment·challenge·Witness·submit·Official/Open·보안 테스트, Gate C는 share·history·rate limit·audit·삭제·운영·CI, Gate D는 P0 전체와 P1 핵심, migration, privacy, methodology, threat model, version 고정을 완료해야 통과한다.
...
> Codex 대화 원문과 로컬 파일은 기본적으로 서버에 업로드되지 않습니다. 공개 요약, 점수, 평가 범위와 evidence commitment만 사용자가 확인한 후 등록됩니다.
